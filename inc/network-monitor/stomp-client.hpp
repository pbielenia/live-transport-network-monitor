...
static const auto stomp_client_error_strings{MakeBimap<StompClientError, std::string_view>({
    // clang-format off
...
std::string_view ToStringView(const StompClientError& command)
{
    ...
    if (string_representation == stomp_client_error_strings.left.end()) {
        return stomp_client_error_strings.left.find(StompClientError::UndefinedError)->second;
    } else {
...
class StompClient {
   public:
     ...
     *                                  The handler receives an error code and the subscription ID.
     *                                  Note: On failure, this callback is only called if the failure
     *                                  happened at the WebSocket level, not at the STOMP level. This
     *                                  is due to the fact that the STOMP server automatically closes
     *                                  the WebSocket connection on a STOMP protocol failure.
     *  \param on_message_callback      Called on every new message from the subscription destination.
     *                                  It is assumed that the message is received with application/json
     *                                  content type.
     * 
     *  All handlers run in a separate I/O execution context from the WebSocket one.
     */
   ...
   private:
    ...
    void OnWebSocketClosed(boost::system::error_code result, std::function<void(StompClientError)> on_close_callback = nullptr);
    ...
    std::function<void(StompClientError)> on_closed)
{
    ...
    websocket_client_.Close([this, &on_closed](auto result){
        OnWebSocketClosed(result, on_closed);
    });
}
...
void StompClient<WebSocketClient>::OnWebSocketConnected(boost::system::error_code result)
{
    if (result.failed()) {
        ...
        CallOnConnectedCallbackWithErrorIfValid(StompClientError::CouldNotConnectToWebSocketServer);
        return;
    }
    ...
    if (error != StompError::Ok) {
        ...
        CallOnConnectedCallbackWithErrorIfValid(StompClientError::UnexpectedCouldNotCreateValidFrame);
        return;
    }
...
void StompClient<WebSocketClient>::OnWebSocketConnectMessageSent(boost::system::error_code result)
{
    if (result.failed()) {
        ...
        CallOnConnectedCallbackWithErrorIfValid(StompClientError::CouldNotSendConnectFrame);
    }
}
                                                              ...
                                                              std::string&& message)
{
    ...
    if (stomp_error != StompError::Ok) {
        ...
        CallOnConnectedCallbackWithErrorIfValid(StompClientError::CouldNotParseMessageAsStompFrame);
        return;
    }
    ...
    std::function<void(StompClientError)> on_close_callback)
{
    if (on_close_callback)  {
        auto error{result.failed() ? StompClientError::CouldNotCloseWebSocketConnection : StompClientError::Ok};
        boost::asio::post(io_context_, [on_close_callback, error]() {
...
template <typename WebSocketClient>
void StompClient<WebSocketClient>::CallOnConnectedCallbackWithErrorIfValid(StompClientError error)
{
    if (on_connected_callback_) {
        boost::asio::post(io_context_, [on_connected_callback = on_connected_callback_, error]() {
            on_connected_callback(error);
        });
    }
}
